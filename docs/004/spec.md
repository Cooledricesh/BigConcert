...
### 1. 예약 정보 검증 규칙
- **공통 검증 스키마**: `/docs/validation.md` 참조
- **예약자명**: 필수, 2-50자, 공백 자동 제거
- **전화번호**: 필수, 10-11자리 숫자 (하이픈 자동 제거)
- **비밀번호**: 필수, 4자리 숫자, bcrypt 해시 처리 후 저장
...
### 9. 세션 관리
- **선택 좌석 정보**: sessionStorage 사용
  - 예약 정보 입력 페이지 진입 시 sessionStorage에서 읽어옴
  - 예약 완료 후 즉시 삭제
  - TTL 체크: 만료된 경우 좌석 선택 페이지로 리다이렉트
- **예약 완료 정보**: sessionStorage + URL param
  - 예약 완료 후 sessionStorage에 예약 정보 저장
  - URL에는 bookingId만 포함 (`/concerts/[id]/confirmation?id=[bookingId]`)

### 10. 에러 처리
- 모든 에러는 명확한 에러 코드와 메시지 반환
- 공통 에러 코드 사용 (`/docs/error-codes.md` 참조)
- 주요 에러 코드:
